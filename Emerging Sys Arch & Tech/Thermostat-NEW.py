...
def log_event_db(conn, mode, temp, set_point):
    ts = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    conn.execute(
        'INSERT INTO readings(timestamp, mode, temperature, set_point) VALUES (?, ?, ?, ?)',
        (ts, mode, temp, set_point)
    )
    conn.commit()
    ...
    def _display_loop(self):
        counter = 1
        toggle = False
        conn = initialize_database()  # Open DB connection
        while not self.endDisplay:
            ...
            if counter % 30 == 0:
                ...
                log_event_db(conn, self.current_state.id, temp, self.setPoint)  # Log to DB
            counter += 1
            sleep(1)
